...
<script>
  ...
  export default {
    ...
    methods: {
      removeInput(port) {
        this.$delete(this.nodes[this.configuring.id].inputs, port);
        ...
        links.map(id => this.$delete(this.links, id));
        this.$delete(this.configuring.inputs, port);
      },
      removeOutput(port) {
        this.$delete(this.nodes[this.configuring.id].outputs, port);
        ...
        links.map(id => this.$delete(this.links, id));
        this.$delete(this.configuring.outputs, port);
      },
      addOutput() {
        ...
        this.$set(this.nodes[this.configuring.id].outputs, newOutput, true);
      ...
      addInput() {
        ...
        this.$set(this.nodes[this.configuring.id].inputs, newInput, true);
        delete this.configuring.newInput;
      },
      updateName() {
        this.$set(this.nodes[this.configuring.id], 'name', this.configuring.name);
      },
      updateSymbol() {
        this.$set(this.nodes[this.configuring.id], 'symbol', this.configuring.symbol);
      },
      deleteNode() {
        // scan links for deletion
        this._.each(this.links, link => {
          if (link.target.node === this.configuring.id) {
            this.$set(this.selectedLinks, link.id, link);
          }
          if (link.source.node === this.configuring.id) {
            this.$set(this.selectedLinks, link.id, link);
          }
