<template>
  <div>
    ...
    <div>
      ...
      <Editor
        :editorStyle="{ height: _.get(editor, 'height', 300) + 'px' }"
        v-if="configuring.module"
      ></Editor>

      <SearchNodes></SearchNodes>

      <div
        class="fixed right-1 bottom-1 gray z-2"
...
export default {
  ...
  computed: {
    panning() {
      return {
        transform: `translate(${this.pan.x}px, ${this.pan.y}px) scale(${
          this.scale
        })`
      };
    }
  ...
  mounted() {
    ...
    dragHandler.onDrag(state => {
      if (
        state.dragging[0] instanceof SVGElement ||
        state.dragging[0].id === "root"
      ) {
